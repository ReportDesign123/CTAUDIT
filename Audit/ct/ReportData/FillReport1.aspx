<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FillReport1.aspx.cs" Inherits="Audit.ct.ReportData.FillReport" %>
...
<head runat="server">
    ...
    var urls={
     ...
     var controls = { AuditPaperTab: {}, AuditReportTab: {}, Tree: "", Nd: "", Zq: "", InsertRow: {}, DeleteRow: {}, ReportAttatch: {},ReportSearch: {} };
     var currentState = { ReportState: { AuditType: { name: "", value: "" }, AuditPaper: { name: "", value: "" }, AuditDate: "", AuditTask: { name: "", value: "" }, Nd: "", Zq: "", ReportType: "", auditPaperVisible: "1", auditZqVisible: "1" },
         navigatorData: { auditReports: [], currentReportId: "",currentReportName:"" },
         RowColChange: true, BdqBh: "", ReportFormat: "", CompanyId: "", BdqDataMaps: {}, IsOrNotWriteLock: "", IsOrNotCheckLock:false,TabDatas: []
     };
     ...
     var EventManager = {
         ...
         ReportSearch: function () {
             ...
             /* var gridIframe = toolsManager.GetGridIframe();
             var para = { TaskId: "", PaperId: "", CompanyId: "", ReportId: "", Year: "", Cycle: "", AuditDate: "", bdqStr: "", Where: "" };
             ...
             var bdqCode = currentState.BdqBh;
             if (bdqCode != "")
             para.Where = "000" + gridIframe.Grid1.selection.FirstRow + "000" + gridIframe.Grid1.selection.FirstCol;*/
         },
         InitializeBtn: function () {
             ...
             controls.ReportSearch = $("#ReportSearch").Btn({ text: "查询", click: EventManager.ReportSearch, icon: "Attatch" });
         ...
         InsertRowCol_Click: function () {
             ...
             if (bdqFormat.BdType == "1") {
                 if (gridFrame.GridManager.GetCurrentRow() == gridFrame.Grid1.Rows - 1) { alert("表格行数需要比插入行数多一行！"); return; };
                 ...
                 gridFrame.Grid1.InsertRow(row, 1);
                 //                 gridFrame.Grid1.Range(row, 1, row, gridIframe.Grid1.Cols - 1).BackColor = toolsManager.CovertColorStr("#FF99CC");
         ...
         DeleteRowCol_Click: function () {
             ...
             if (bdqFormat.BdType == "1") {
                 ...
                 var bdFlag = false;
                 for (var i = 1; i < gridFrame.Grid1.Cols; i++) {
                     var tag = gridFrame.Grid1.Cell(row, i).Tag;
                     if (tag != "") {
                         if (tag.substr(0, 1) == "0") {
                             bdFlag = true; break;
                         }
                 ...
                 var colNum = gridFrame.Grid1.Cols;
                 var col;
                 for (var i = 1; i < colNum; i++) {
                     var cellTag = gridFrame.Grid1.Cell(row, i).Tag;
                     if (cellTag != "") { col = i; break; }
                 }
                 var bdInfo = toolsManager.GetCellTagBdqInfo(row, col);

                 gridFrame.Grid1.Range(row, 1, row, colNum - 1).DeleteByRow();
                 ...
                 var bdFlag = false;
                 for (var i = 1; i < gridFrame.Grid1.Rows; i++) {
                     var tag = gridFrame.Grid1.Cell(i, col).Tag;
                     if (tag != "") {
                         if (tag.substr(0, 1) == "0") {
                             bdFlag = true; break;
                         }
                 ...
                 var rowNum = gridFrame.Grid1.Rows;

                 var row;
                 for (var i = 1; i < rowNum; i++) {
                     var cellTag = gridFrame.Grid1.Cell(i, col).Tag;
         ...
         MenuManager: {
             SaveGrid: function () {
                 ...
                 for (var i = 0; i < gridIframe.Grid1.Rows; i++) {

                     for (var j = 0; j < gridFrame.Grid1.Cols; j++) {

                         var Cell = gridFrame.Grid1.Cell(i, j);
                         ...
                         if (tagStr != "") {
                             ...
                             if (gdBdArr[0] == "1") {
                                 ...
                                 var cellValue = gridFrame.GridManager.GetRowColTextByCellType(Cell.Text, cellFormat);
                                 ...
                                 if (BBDataItems.BdqData[bdDataIndex][bdTagInfo.index][cellCode].value != cellValue) {
                                     //  BBDataItems.BdqData[bdDataIndex][bdTagInfo.index][cellCode].value = cellValue;
             ...
             LockCell: function () {
                 ...
                 var gridIframe = toolsManager.GetGridIframe();
                 //锁定单元格
                 gridIframe.Grid1.Cell(gridIframe.Grid1.selection.FirstRow, gridIframe.Grid1.selection.FirstCol).Locked = true;
                 gridIframe.Grid1.Cell(gridIframe.Grid1.selection.FirstRow, gridIframe.Grid1.selection.FirstCol).BackColor = toolsManager.CovertColorStr("#FF99CC");
     ...
     var menus = [
     { text: "文件", icon: 'archives', menu: { items: [
     ...
     { text: "导出Excel", click: EventManager.MenuManager.ExportExcel, icon: 'excil'}//  , icon: 'archives'
        ...
        { text: "本级审核", icon: 'ok', menu: {
         ...
         { text: "取消锁定", click: EventManager.MenuManager.SaveGrid, icon: 'NoLock' }
         ...
         $(function () {
             ...
             var tempState = CookieDataManager.GetCookieData(ReportDataAction.Functions.FillReport);
         ...
         var toolsManager = {
             SetAuditTask: function (result) {
                 ...
                 //                 mediatorManager.LoadAuditPaperAndReport();
                 //                 mediatorManager.LoadCompanies();
                 //                 mediatorManager.LoadZq();
         ...
         var mediatorManager = {
             ...
             LoadBbData: function () {
                 ...
                 if (bdqCode != "")
                     para.Where = gridIframe.currentState.Row + "&" + gridIframe.currentState.Col + "&" + $("#txtSearch").val();
                 ...
                 if (tag != "" && tag.length > 0) {
                     ...
                     if (bdArr[0] == "0") {
                         ...
                         if (currentState.BdqDataMaps[bdInfo.bdCode] && currentState.BdqDataMaps[bdInfo.bdCode].isOrNotLoadAll == "0") {
                             $("#LoadInf").remove();
                             $("#Tool").append("<div id='LoadInf'><div id='BdNumber' style = 'margin-left:5px;float:left'>当前加载 " + currentState.BdqDataMaps[bdInfo.bdCode].loadNumber + "行(列),共" + currentState.BdqDataMaps[bdInfo.bdCode].totalNumber + "行(列) </div><div id='LoadAll' style='margin-left:10px'></div></div>");
                             $("#LoadAll").Btn({ text: "全部加载", click: EventManager.BdqLoadAllData_Click, icon: "Attatch" });
                         } else {
             ...
             RefreshReport: function () {
                 ...
                 if (BBData.bdq.bdNum > 0) {
                     ...
                     for (var i = 1; i < gridFrame.Grid1.Rows; i++) {
                         for (var j = 1; j < gridFrame.Grid1.Cols; j++) {
                             var tag = gridFrame.Grid1.Cell(i, j).Tag;
                             if (tag == "") {
                                 gridFrame.Grid1.Cell(i, j).Locked = true;
                             }
                         }
                     }
                 } else {
                 ...
                 if (data.success) {
                     if (data.obj) {
                         ...
                         $.each(data.obj.Gdq, function (cellCode, CellItem) {
                             gridFrame.GridManager.SetRowColText(CellItem);
                         });
                         ...
                         $.each(data.obj.BdqData, function (bdqIndex, bdqData) {
                             ...
                             if (bdFormat.BdType == "1") {
                                 ...
                                 if (rowNum > 1) {
                                     gridIframe.Grid1.AutoRedraw = false;
                                     gridIframe.Grid1.InsertRow(bdFormat.Offset + 1 + addRowData, rowNum - 1);
                                     gridIframe.Grid1.AutoRedraw = true;
                                     //gridFrame.Grid1.Range(bdFormat.Offset+addRowData + 1, 1, bdFormat.Offset + rowNum+addRowData - 1, gridIframe.Grid1.Cols - 1).BackColor = toolsManager.CovertColorStr("#FF99CC");
                                     gridIframe.Grid1.AutoRedraw = false;
                                     //设置变动行中的数据格式信息
                                     $.each(BBData.bbData[bdFormat.Offset], function (columnIndex, cell) {
                                         for (var i = bdFormat.Offset + 1 + addRowData; i <= bdFormat.Offset + rowNum + addRowData - 1; i++) {
                                             var cellCode = cell["CellCode"];
                                             if (cellCode) {
                                                 ...
                                                 gridFrame.Grid1.Cell(i, columnIndex).Tag = CellTag;

                                                 //设置单元格对齐方式
                                                 if (cell.CellDataType == "01") {
                                                     gridFrame.Grid1.Cell(i, columnIndex).Alignment = 4;
                                                 } else if (cell.CellDataType == "02") {
                                                     gridFrame.Grid1.Cell(i, columnIndex).Alignment = 12;
                                                 }
                                             }
                                         }
                                     });
                                         ...
                                         $.each(BBData.bbData[bdFormat.Offset], function (columnIndex, cell) {
                                             if (cell.CellCode) {
                                                 ...
                                                 if (bdRow && bdRow[cell.CellCode]) {
                                                     gridFrame.GridManager.SetRowColTextByRowCol(row, columnIndex, bdRow[cell.CellCode]);
                                                 } else {
                         ...
                         if (data.obj.rdps.ReportId != "home") {
                             ...
                             //if (report.ReportState == "报表数据审批中" || report.ReportState == "报表审批通过" || report.ReportState == "报表审批不通过") {
                             //    currentState.IsOrNotCheckLock = true;
                             //}
                         }
                       gridIframe.Grid1.AutoRedraw = true;
                       gridIframe.Grid1.Refresh();
                     ...
                     $("#LoadInf").css("display", "none");
                     $("#LoadAll").css("display", "none");
                     for (var i = 1; i < gridIframe.Grid1.Cols; i++) {
                         gridIframe.Grid1.Cell(0, i).Text = i;
                     }
                 } else {
                     alert(data.sMeg);
                 }
...
<body style=" overflow:hidden">
   ...
   <%--         <div id="AuditPaperTab" class="AuditPaperTab">
            ...
            <div id="Tool" class="l-layout-header">
                ...
                <span style="color: #A1A1A1; font-size: 16px; float: left; line-height: 16px; margin: 2px;
